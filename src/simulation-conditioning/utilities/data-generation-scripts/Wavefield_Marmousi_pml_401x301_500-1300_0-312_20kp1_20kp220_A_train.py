...
datasize0 = int(np.shape(range(1, shape[0], 20))[0])
datasize1 = int(np.shape(range(220, nt, 20))[0])
datasize = datasize0*datasize1


strTrainA = os.path.join(save_dir, 'Wavefield_Marmousi_pml_401x301_500-1300_0-312_20kp1_20kp220_A_train.hdf5')
strTrainB = os.path.join(save_dir, 'Wavefield_Marmousi_pml_401x301_500-1300_0-312_20kp1_20kp220_B_train.hdf5')
...
kk = 0
for xsrc in range(1, shape[0], 20):
    ...
    datasetA[kk:(kk+datasize1), :, :] = np.array(ulocgood.data[range(220, nt, 20), :, :])
    datasetB[kk:(kk+datasize1), :, :] = np.array(ulocbad.data[range(220, nt, 20), :, :])
