...
datasize0 = int(np.shape(range(0, shape[0], 6))[0])
datasize1 = int(np.shape(range(100, nt, 30))[0])
datasize = datasize0*datasize1


strTrainA = os.path.join(save_dir, 'Wavefield_Marmousi_pml_401x301_1013-1300_38-150_6k_30kp100_A_train.hdf5')
strTrainB = os.path.join(save_dir, 'Wavefield_Marmousi_pml_401x301_1013-1300_38-150_6k_30kp100_B_train.hdf5')
...
for xsrc in range(0, shape[0], 6):
    ...
    datasetA[kk:(kk+datasize1), :, :] = np.array(ulocgood.data[range(100, nt, 30), :, :])
    datasetB[kk:(kk+datasize1), :, :] = np.array(ulocbad.data[range(100, nt, 30), :, :])
