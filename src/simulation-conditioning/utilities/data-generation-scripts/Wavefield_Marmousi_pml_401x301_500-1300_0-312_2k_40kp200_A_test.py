...
datasize0 = int(np.shape(range(0, shape[0], 2))[0])
datasize1 = int(np.shape(range(200, nt, 40))[0])
datasize = datasize0*datasize1


strTrainA = os.path.join(save_dir, 'Wavefield_Marmousi_pml_401x301_500-1300_0-312_2k_40kp200_A_test.hdf5')
strTrainB = os.path.join(save_dir, 'Wavefield_Marmousi_pml_401x301_500-1300_0-312_2k_40kp200_B_test.hdf5')
...
for xsrc in range(0, shape[0], 2):
    ...
    datasetA[kk:(kk+datasize1), :, :] = np.array(ulocgood.data[range(200, nt, 40), :, :])
    datasetB[kk:(kk+datasize1), :, :] = np.array(ulocbad.data[range(200, nt, 40), :, :])
